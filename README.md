...
**Tous les fichiers sont à plat, à la racine du dépôt** (aucun sous-dossier).
C'est volontaire : l'upload web de GitHub aplatit souvent les sous-dossiers,
ce qui casse les chemins `js/…` / `css/…` et fait échouer le chargement des
scripts (404) sans message clair. En gardant tout à plat, un simple
glisser-déposer de l'ensemble des fichiers fonctionne à tous les coups.
...
└── icon-maskable-512.png Icône PWA 512 maskable
```
...
1. Sur la page du repo fraîchement créé, cliquez sur **uploading an
   ...
   pas).
2. **Sélectionnez tous les fichiers du dossier `Babyphone/`** (ils sont
   tous à plat, aucun sous-dossier) et glissez-les dans la zone d'upload.
   Vérifiez que la liste des fichiers à envoyer contient bien les 19
   fichiers applicatifs listés dans l'arborescence ci-dessus, notamment
   tous les `.js`, `style.css` et les 3 `.png` : s'il
   en manque, GitHub Pages renverra des 404 et l'app se chargera sans
   fonctionner.
